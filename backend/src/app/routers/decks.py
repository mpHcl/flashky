...
@router.post("/decks")
def createDeck(deck_data: DeckDTO, user_id: int = Depends(authenticate()), db: Session = Depends(get_session)):
    user_id = int(user_id)
    if not user_id:
...
def deleteDeck(deck_id: int, user_id: int = Depends(authenticate()), db: Session = Depends(get_session)):
    user_id = int(user_id)
    if not user_id:
        raise HTTPException(status_code=404, detail="User not found")
...
def updateDeck(deck_id: int, deck_data: DeckUpdateDTO, user_id: int = Depends(authenticate()), db: Session = Depends(get_session)):
    user_id = int(user_id)
    if not user_id:
        raise HTTPException(status_code=404, detail="User not found")
